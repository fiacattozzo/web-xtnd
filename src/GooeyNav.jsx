...
const GooeyNav = ({
  ...
  const handleClick = (e, index) => {
    ...
    // dejamos el comportamiento de <a> para que haga scroll al anchor
  };
    ...
    <>
      <style>
        {`
          ...
          li::after {
            content:""; position:absolute; inset:0; border-radius:8px; background:white;
            opacity:0; transform:scale(0); transition: all 0.3s ease; z-index:-1;
          }
        `}
      </style>
        ...
        <nav className="flex relative" style={{ transform: "translate3d(0,0,0.01px)" }}>
          ...
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`rounded-full relative cursor-pointer transition-colors duration-300 ease-in-out text-white ${activeIndex === index ? "active" : ""}`}
              >
                <a
